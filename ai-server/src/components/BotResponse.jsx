...
export const BotResponse = ({ response, isLoading, onRetry, error }) => {
  ...
  return (
    <div className="bg-dark-gray border border-neon-green/30 rounded-lg overflow-hidden mb-6 animate-fade-in">
      {/* Tabs */}
            ...
            {response.snapshot && (
              <div className="mt-8 bg-[#1a1a1a] border border-gray-800 rounded-xl overflow-hidden">
                <div className="bg-[#242424] px-4 py-3 border-b border-gray-800">
                  <h4 className="text-white font-semibold text-sm">Current snapshot (as of {new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-')})</h4>
                </div>
                <table className="w-full text-sm text-left">
                  ...
                  <table className="w-full text-sm text-left min-w-[600px]">
                    ...
                    <tbody>
                      {response.events.map((event, i, arr) => {
                        // We map the old "name" and "impact" to this new 4-column structure 
                        // by using some mock logic if the event doesn't exactly fit
                        ...
                        return (
                          <tr key={i} className={i !== arr.length - 1 ? "border-b border-gray-800 hover:bg-[#222] transition-colors" : "hover:bg-[#222] transition-colors"}>
                            ...
                            <td className="py-4 px-4 text-gray-400 leading-relaxed">{importanceDesc}</td>
                          </tr>
                        )
                      })}
                    </tbody>
