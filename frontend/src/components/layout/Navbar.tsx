        ...
        <div className="h-16 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-10">
            ...
            <div className="flex items-center space-x-6">
                ...
                <div className="flex items-center space-x-4">
                    {/* Wallet address chip — only shown for MetaMask users */}
                    {shortWallet && (
                        <button
                            onClick={copyAddress}
                            title={copied ? 'Copied!' : `Full address: ${user?.walletAddress}`}
                            className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-orange-500/30 bg-orange-500/10 text-orange-400 text-xs font-mono font-bold hover:bg-orange-500/20 hover:border-orange-400/50 transition-all duration-200 group"
                        >
                            <Wallet className="w-3 h-3" />
                            <span>{copied ? 'Copied!' : shortWallet}</span>
                        </button>
                    )}
