...
export function Navbar() {
    ...
    React.useEffect(() => {
        const hotTopics = [
            "Hot Topic: Tech stocks are rallying globally!",
            "Hot Topic: TSLA announced highly anticipated Q4 earnings.",
            "Hot Topic: Federal Reserve hints at upcoming rate cuts.",
            "Hot Topic: Energy sector sees a sharp influx of retail investors.",
            "Hot Topic: Bitcoin breaks new resistance levels this week.",
            "Hot Topic: Analysts upgrade LENSKART to Strong Buy."
        ];

        const intervalId = setInterval(() => {
            const randomTopic = hotTopics[Math.floor(Math.random() * hotTopics.length)];
            addNotification('info', randomTopic);
        }, 15000); // Fire every 15 seconds to simulate high activity
