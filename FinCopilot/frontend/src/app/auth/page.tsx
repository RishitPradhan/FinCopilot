...
export default function AuthPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const storedUsersStr = localStorage.getItem('fincopilot_users');
            ...
            if (user) {
                // remove strict password before assigning to state
                const { password, ...safeUser } = user;
                setAuth(safeUser as any, 'mock-local-token-' + Date.now());
                addNotification('success', `Welcome back, ${safeUser.name}!`);
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (storedUsers.some((u: any) => u.email === formData.email)) {
                addNotification('error', 'User with this email already exists.');
                return;
            }
            ...
            const newUser = {
                ...
                password: formData.password, // Only storing here for mock local authentication logic
                riskAppetite: formData.riskAppetite
            };

            storedUsers.push(newUser);
            localStorage.setItem('fincopilot_users', JSON.stringify(storedUsers));
