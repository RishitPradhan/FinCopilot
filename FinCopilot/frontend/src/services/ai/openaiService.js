...
if (!apiKey || apiKey === 'your_openai_api_key_here') {
  console.warn('OpenAI API key not found. Ensure NEXT_PUBLIC_OPENAI_API_KEY is in .env.local');
}
...
export const sendMessageToGPT = async (userMessage, fileData = null, model = 'gpt-4o') => {
  // Check if API key is available
  if (!apiKey || apiKey === 'your_openai_api_key_here') {
    throw new Error('Please add your OpenAI API key to the .env.local file. Copy from .env.example')
  }
